...
class FuguJPNTranslator:
    def __init__(self, marian_ports, use_sentence_tokenize=True, use_constituency_parsing=True,
        ...
        if use_sentence_tokenize:
            if use_sentence_tokenize == 'pysbd':
                import pysbd

                def sent_tokenize_en(txt):
                    seg_en = pysbd.Segmenter(language="en", clean=False)
                    return [v for v in filter(lambda x: len(x) > 0, [clean_txt(s) for s in seg_en.segment(txt.strip())])]
        ...
        lid_filepath = os.path.join(nltk.downloader.Downloader().default_download_dir(), 'lid.176.bin')
        if not os.path.exists(nltk.downloader.Downloader().default_download_dir()):
            os.makedirs(nltk.downloader.Downloader().default_download_dir())
        if not os.path.exists(lid_filepath):
            urllib.request.urlretrieve('https://dl.fbaipublicfiles.com/fasttext/supervised-models/lid.176.bin', lid_filepath)
        fastext_model = fasttext.load_model(lid_filepath)

        nltk.download('punkt')
  
        def is_ja_score(txt):
            if fastext_model.predict(txt)[0][0] == '__label__ja':
                return 1.0
            else:
                return 0.5

        self.is_ja_score = is_ja_score
        ...
        try:
            ...
            for ja_text in ja_text_list:
                ...
                for idx in range(translate_sentence_length):
                    ...
                    ej_score = cos_sim(en_vecs[idx], ja_vecs[idx]) * min(word_rate / 0.85, 0.85 / word_rate) * self.is_ja_score(jns)
                    ...
                    if idx in idx_partitioned_idx:
                        ...
                        word_rate = self.comp_en_ja_tokens(ens, jns)
                        ej_score = cos_sim(en_vecs[idx], ja_concat_vec) * min(word_rate / 0.85, 0.85 / word_rate) * self.is_ja_score(jns)
